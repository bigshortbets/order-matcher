...
const manageOrders = async (
  ...
  while (
    sortedLongOrderCollection.size > 0 &&
    sortedShortOrderCollection.size > 0
  ) {
    ...
    for (const shortOrder of sortedShortOrderCollection) {
      if (
        ...
        shortOrder.price <= nextLong.price
      ) {
  ...
  return new Set(
    orders
      ...
      .sort((a: Order, b: Order) => {
        return BigInt(a.price)
          .toString()
          .localeCompare(BigInt(b.price).toString());
      })
  );
};
